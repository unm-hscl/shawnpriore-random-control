function [p, traj] = verify(N, x_0, U, A, B, G, h, time_horizon, a, b)
    tester = zeros(N,1);
    ...
    traj = zeros( size(A, 1)*time_horizon, N);
    ...
    for  sample = 1:N
        for i = 0:(time_horizon-1)
            C_concat(size(B, 1)*i + [1:size(B, 1)], size(B, 2)*i + [1:size(B, 2)]) = B * diag(betarnd(a, b, 1, 3));
        end
